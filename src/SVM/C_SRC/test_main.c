...
static double classify(gsl_vector* _u, gsl_vector* _w, double b);
...
PyObject* __test_t(PyObject* elements, PyObject* w, PyObject* b) {
    ...
    for (Py_ssize_t i=0; i<PyList_Size(elements); i++) {
        ...
        PyList_SetItem(pred_values, i, PyFloat_FromDouble(classify(unwrap_u(PyObject_GetAttrString(curr_e, "features")), _w, _b)));
